...
Run:  python examples/scoring/glide_batch.py \\
          ...
          [--precision HTVS] [--ph 7.4] [--no-epik]

Or, to resolve the grid/precision/ph from a docking_settings.json entry instead
of passing them directly (see examples/docking_settings.json):

      python examples/scoring/glide_batch.py \\
          --proposals <file> --out <file> \\
          --docking-settings examples/docking_settings.json --target <NAME>
"""
...
def resolve_from_docking_settings(settings_path: Path, target: str) -> tuple[Path, str, float | None]:
    """Resolve (grid_path, precision, ph) for `target` from a docking_settings.json file.
    ...
    grid_line = next(
        (line for line in in_path.read_text().splitlines() if line.strip().startswith("GRIDFILE")),
        None,
    )
    if grid_line is None:
        raise SystemExit(f"no GRIDFILE line found in {in_path}")
    grid_rel = grid_line.split()[1]
    return repo_root / grid_rel, entry["precision"], entry.get("ph")
...
def run_ligprep(*, ligands: Path, workdir: Path, ph: float, epik: bool, schrodinger: str) -> Path:
    ...
    prepped = workdir / "ligands_prepped.mae"
    cmd = [str(ligprep), "-WAIT", "-ismi", str(ligands), "-omae", str(prepped), "-ph", str(ph)]
    if epik:
        cmd.append("-epik")
...
def run_glide(*, ligands: Path, grid: Path, workdir: Path, precision: str, schrodinger: str) -> Path:
    ...
    # Glide writes <jobname>.csv with an 'r_i_docking_score' column.
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.docking_settings:
        ...
        settings_grid, settings_precision, settings_ph = resolve_from_docking_settings(args.docking_settings, args.target)
    ...
    if not todo.empty:
        if not schrodinger or not (Path(schrodinger) / "glide").exists():
            ...
            results = run_glide(
                ...
                precision=precision, schrodinger=schrodinger,
            )
            docked = pd.read_csv(results)
            known_ids = set(todo["product_id"].astype(str))
            score_by_id = fold_epik_states(docked, args.score_column, known_ids)
            for _, row in todo.iterrows():
